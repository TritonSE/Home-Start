...
async function seed() {
  ...
  await Promise.all([
    volunteerAssignmentModel_1.default.deleteMany({}),
    projectProgramMapModel_1.default.deleteMany({}),
    volunteerModel_1.default.deleteMany({}),
    tagModel_1.default.deleteMany({}),
  ]);
  const [assignmentTag, shiftTag] = await tagModel_1.default.create([
    { name: "Front Desk", color: "#4F46E5", type: "assignment" },
    { name: "Morning Shift", color: "#D97706", type: "shift" },
  ]);
  const [groupTagNorth, groupTagWest] = await tagModel_1.default.create([
  ...
  const projectTags = await tagModel_1.default.create(
  ...
  const programTags = await tagModel_1.default.create(
  ...
  await projectProgramMapModel_1.default.create(
  ...
  const volunteerOne = await volunteerModel_1.default.create({
    firstName: "Mia",
    lastName: "Fernandez",
  ...
  const volunteerTwo = await volunteerModel_1.default.create({
    firstName: "Jordan",
    lastName: "Lee",
  ...
  const [
    ...
    volunteerEleven,
    volunteerTwelve,
  ] = await volunteerModel_1.default.create([
    {
      firstName: "Sophia",
      lastName: "Patel",
  ...
  await volunteerAssignmentModel_1.default.create([
    {
  ...
  // After assignments are created, pick a single program tag per volunteer
  // (choose the first program encountered for that volunteer) and persist
  // it to the volunteer's `programTagId` field.
  const assignments = await volunteerAssignmentModel_1.default.find();
  const projectProgramMaps = await projectProgramMapModel_1.default.find();
  const projectToProgram = new Map();
  for (const map of projectProgramMaps) {
    projectToProgram.set(String(map.projectTagId), String(map.programTagId));
  }
  const volunteerFirstProgram = new Map();
  for (const a of assignments) {
    const vid = String(a.volunteerId);
    const projectTag = String(a.projectTagId);
    const programTag = projectToProgram.get(projectTag);
    if (programTag && !volunteerFirstProgram.has(vid)) {
      volunteerFirstProgram.set(vid, programTag);
    }
  }
  const updateOps = [];
  for (const [volId, programTagId] of volunteerFirstProgram.entries()) {
    updateOps.push(volunteerModel_1.default.findByIdAndUpdate(volId, { $set: { programTagId } }));
  }
